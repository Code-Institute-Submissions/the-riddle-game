...
def final_score(username, score):
    ...
    if username != "" and score != "":
        with open("data/scores.txt", "a") as file:
            ...
            if int(score) > 0 and int(score) < 8:
                score = "0" + str(score)
...
def index():
    ...
    if request.method == "POST":
        ...
        if username == "":
            ...
            return redirect(url_for('game1', username=username, score=score))
    return render_template("index.html")


@app.route('/<username>/game1/<int:score>', methods=["GET", "POST"])
def game1(username, score):
    """ User starts with 3 guesses / game """
    rem_attempts = 3
    if request.method == "POST":
        ...
        if answers[0] == guess:
            ...
            if num_of_attempts() > 2:
                ...
                """ If answer is incorrect but they've not had more than 3 guesses, reload the current game """
                return render_template("game1.html", username=username, attemp=get_all_attempts(), rem_attempts=attempts_rem(), score=score)
...
def score():
    ...
    return render_template("score.html", usernames_and_scores=usernames_and_scores)
